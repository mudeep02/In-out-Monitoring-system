...
class CentroidTracker:
    def __init__(self, max_disappeared=40):
        ...
        self.track_age = OrderedDict()
        self.max_disappeared = max_disappeared

    def register(self, centroid):
        self.objects[self.next_object_id] = centroid
        self.disappeared[self.next_object_id] = 0
        self.track_age[self.next_object_id] = 1
        self.next_object_id += 1

    def deregister(self, object_id):
        del self.objects[object_id]
        del self.disappeared[object_id]
        del self.track_age[object_id]
...
# --------------------- Model ---------------------
model = YOLO("yolov8m.pt")  # ⬅️ Upgrade to yolov8m.pt for 99% accuracy
...
while True:
    ...
    for r in results:
        for box in r.boxes:
            ...
            if cls == 0 and conf > 0.6:
                ...
                if any(compute_iou([x1, y1, x2, y2], b) > 0.5 for b in boxes):
                    continue
    ...
    cv2.imshow("People Counting - 99% Accuracy", frame)
